...
include "connection.php";
 ...
 if($_SESSION['rtype']=="standard")
	...
	while($s1=mysql_fetch_array($rs))
		   ...
		   {
		   		 $se="select max(booked_rooms)from reservation where in_date='$_SESSION[in]' and out_date='$_SESSION[out]'";
	...
	while($s1=mysql_fetch_array($rs))
			{
			  ...
			  $x=$broom+$_SESSION['room'];
			     if ($in==$_SESSION['in'] && $out==$_SESSION['out'])
		   {
  $sql="insert into reservation (room_type,in_date,out_date,rooms,booked_rooms,adults,childrens)values ('$_SESSION[rtype]','$_SESSION[in]','$_SESSION[out]',$_SESSION[room],$x,$_SESSION[adlt], $_SESSION[ch])";
 mysql_query($sql,$con);
